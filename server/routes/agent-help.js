  ...
  res.json({
    ...
    endpoints: {
      ...
      images: {
        ...
        'GET /api/schedule-image/yearly?year=Y&endYear=Y': '生成多个年份的年课表图片（纵向堆叠），endYear 为可选结束年份；支持 theme 参数',
      },
      backup: {
    ...
    notes: [
      ...
      'GET /api/backup 返回全量 JSON（含 version 字段，当前为 1）；POST /api/backup/restore 校验 version 必须匹配，不匹配返回 400；恢复前必须成功保存快照到 ./data/.backup_pre_restore_<uuid>.json，否则返回 500 并中止还原；恢复过程中 teacherId 强制绑定当前账号；schedules、classStudents、classPricing 关联会校验 classId 是否存在于恢复数据中，classStudents 额外校验 studentId，无效关联自动跳过',
    ...
    validationRules: {
      ...
      schedules: {
        create: { classId: '整数必填', date: 'YYYY-MM-DD 必填', startTime: 'HH:MM 必填（00:00-23:59）', endTime: 'HH:MM 必填（推荐 00:00-23:59，可传 24:00-47:59；endTime < startTime 表示跨午夜）', durationBilling: '可选非负整数（默认由 endTime-startTime 自动计算）', locationName: '可选字符串（默认继承班级默认地点）', locationLat: '可选数字', locationLng: '可选数字' },
        update: { classId: '可选整数（变更时校验归属和软删除）', date: '可选 YYYY-MM-DD', startTime: '可选 HH:MM（00:00-23:59）', endTime: '可选 HH:MM（推荐 00:00-23:59，可传 24:00-47:59；endTime < startTime 表示跨午夜）', durationBilling: '可选非负整数（startTime/endTime 变更且未显式传时自动重算）', locationName: '可选字符串', locationLat: '可选数字', locationLng: '可选数字' },
        batchCreate: { classId: '整数必填', startTime: 'HH:MM 必填（00:00-23:59）', endTime: 'HH:MM 必填（推荐 00:00-23:59，可传 24:00-47:59；endTime < startTime 表示跨午夜）', dates: '可选数组,最多 365 项', semesterId: '可选,与 weekday 配合学期模式', weekday: '可选 0-6,与 semesterId 配合', durationBilling: '可选非负整数（默认由 endTime-startTime 自动计算）', preview: '布尔值可选（默认 false,true 时仅返回 {count,dates} 预览不创建）' },
