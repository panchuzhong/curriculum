...
router.get('/agent/help', (req, res) => {
  res.json({
    ...
    notes: [
      ...
      '审计日志（audit_log）覆盖：排课 CREATE/UPDATE/DELETE/BATCH_DELETE/BATCH_UPDATE；班级 CREATE/UPDATE/DELETE；学生 CREATE/UPDATE/DELETE（含子路由 POST/DELETE /api/classes/:cid/students）；节假日 CREATE/UPDATE/DELETE/BATCH_CREATE。定价阶梯与学期暂未写入审计日志。日志超过 10000 条时自动删除最旧记录',
    ...
    validationRules: {
      ...
      classes: {
        create: { name: '必填', grade: '枚举：初一/初二/初三/高一/高二/高三/大学', subject: '必填', studentCount: '≥1整数', unitPrice: '可选≥0', discountAmount: '可选≥0', isCompetition: '可选布尔' },
        update: { name: '可选（不为空）', grade: '可选枚举', subject: '可选（不为空）', studentCount: '可选≥1', unitPrice: '可选≥0', discountAmount: '可选≥0' },
        addStudent: { name: '必填', phone: '可选 11 位手机号（与 POST /api/students 一致）', parentPhone: '可选 11 位手机号', note: '可选自由文本' },
      },
      students: {
        ...
        update: { name: '可选', phone: '可选11位', birthDate: '可选 YYYY 或 YYYY-MM-DD', classIds: '可选整数数组（无效班级 ID 静默跳过）' },
      },
      schedules: {
        ...
        batchCreate: { classId: '整数必填', startTime: 'HH:MM 必填', endTime: 'HH:MM 必填', dates: '可选数组,最多 365 项', semesterId: '可选,与 weekday 配合学期模式', weekday: '可选 0-6,与 semesterId 配合' },
        batchUpdate: { classId: '整数必填', fromDate: 'YYYY-MM-DD 可选', weekday: '0-6 可选（省略匹配所有星期几）', updates: '对象必填', note: 'fromDate 与 weekday 至少传一个' },
