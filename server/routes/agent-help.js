  ...
  res.json({
    ...
    endpoints: {
      ...
      schedules: {
        ...
        'PUT /api/schedules/batch': '批量更新排课时间/地点，或把日期整体平移（见 batchUpdateMode）',
        ...
        'GET /api/schedules/free-slots?date=YYYY-MM-DD': '查询单日空闲时段（默认 08:00-22:30；可用 after=HH:MM&before=HH:MM 限制时段;不支持跨午夜,after≥before 返回 400;date 与时间参数格式非法返回 400）；返回 {date, slots:[{start,end}]}',
      ...
      holidays: {
        ...
        'POST /api/holidays/batch': '批量导入节假日（需 items 数组，每项 date/type 必须有效），返回 {count, skipped}（已存在或批内重复日期计入 skipped）',
      },
      pricingTiers: {
      ...
      images: {
        'GET /api/schedule-image?start=YYYY-MM-DD&end=YYYY-MM-DD': '生成周课表 PNG 图片（返回 image/png），最多 31 天；教师无班级时返回 404 {error:"No classes"}；渲染失败时返回 500 {error:"Image generation failed"}',
        ...
        'GET /api/schedule-image?start=&end=&scale=N': '整体缩放因子（0.25-4，默认 3），scale=1 适合手机查看，scale=0.5 更小体积',
        ...
        'GET /api/schedule-image/monthly?year=Y&month=M&endYear=Y&endMonth=M': '生成多个月份的月课表图片（纵向堆叠），endYear/endMonth 为可选结束范围，最多 24 个月',
        'GET /api/schedule-image/yearly?year=Y': '生成年课表概览 PNG 图片（返回 image/png），12 个月卡片 + 年度统计；支持 theme=light|dark|auto（默认 auto，同周课表）',
        'GET /api/schedule-image/yearly?year=Y&endYear=Y': '生成多个年份的年课表图片（纵向堆叠），endYear 为可选结束年份，最多 12 年；支持 theme 参数',
      },
      backup: {
    ...
    batchScheduleModes: {
      semester: {
        ...
        params: {
          ...
          startTime: '开始时间 HH:MM（必填，00:00-23:59）',
          endTime: '结束时间 HH:MM（必填，推荐 00:00-23:59；可传 24:00-47:59，保存时归一为钟表时间；endTime < startTime 表示跨午夜）',
      ...
      dates: {
        description: '按指定日期列表批量排课，不自动跳过节假日；日期跨学期边界（部分在学期内、部分在学期外）时默认返回 400 错误（可传 crossSemester:true 绕过）',
        params: {
          classId: '班级 ID（必填）',
          ...
          durationBilling: '计费时长分钟，留空自动计算（可选）',
          preview: '布尔值（可选，默认 false），true 时仅返回 {count, dates} 不实际创建',
          crossSemester: '布尔值（可选，默认 false），true 时跳过跨学期校验，允许日期跨越学期边界',
        },
        response: '{ count: N, ids: [...] }；preview=true 时返回 {count, dates} 仅预览不创建；日期跨学期且未传 crossSemester:true 时返回 400',
      },
    },
    batchDeleteModes: {
      byIds: {
    ...
    batchUpdateMode: {
      description: 'PUT /api/schedules/batch — 批量修改同一班级在指定范围内的排课的时间/地点，或把它们的日期整体平移若干天',
      params: {
        classId: '班级 ID（必填）',
        fromDate: '起始日期 YYYY-MM-DD（含，可选）；只修改该日期及之后的课。fromDate、toDate 与 weekday 至少传一个',
        toDate: '截止日期 YYYY-MM-DD（含，可选）；只修改该日期及之前的课。未传 fromDate 时下界默认为今天。fromDate、toDate 与 weekday 至少传一个。未传 fromDate 只传 toDate 时 fromDate 默认为今天',
        ...
        updates: '要修改的字段对象（必填）；常量字段：startTime, endTime, durationBilling, locationName, locationLat, locationLng；另有 dayShift（整数，-3650~3650）把每节课的日期各自平移这么多天，可与常量字段同时使用',
        dryRun: '只预览不写库（boolean，默认 false）；返回与真实调用一致的 count/ids/semesterFiltered/holidayDates，并在 dayShift 时附带 dates:[{id,from,to}]。校验与冲突检查照常执行，所以预览看到的 400/409 就是真跑一次的结果',
      },
      allowedUpdateFields: ['startTime', 'endTime', 'durationBilling', 'locationName', 'locationLat', 'locationLng', 'dayShift'],
      note: 'startTime/endTime 变更时自动重算 durationBilling（除非 updates 中显式传 durationBilling）；不在 allowedUpdateFields 中的字段静默忽略。dayShift 是相对位移而非赋值：没有 date 字段可以批量赋值，因为把 date 当常量写就是把整批课压到同一天。「把周四的课统一挪到周五」= weekday:4 + updates:{dayShift:1}；「整体推迟一周」= dayShift:7。位移后跑出学期的那几节按 semesterOnly 规则留在原地（计入 semesterFiltered），排课 id 始终保持不变',
      response: '{ count: N, ids: [...] }；跨学期被过滤时附加 semesterFiltered 与 hint 字段；dayShift 后有课落在节假日时附加 holidayDates（日期数组，只提示不拦截）。位移目标与该班级已有的课重复时返回 409 且一行都不修改',
      example: '{"classId":1,"fromDate":"2026-05-01","weekday":6,"updates":{"startTime":"14:00","endTime":"16:00"}}',
      exampleDayShift: '{"classId":1,"weekday":4,"dryRun":true,"updates":{"dayShift":1}} — 先预览：周四的课全部挪到周五',
    },
    summaryResponse: {
    ...
    notes: [
      ...
      'PUT /api/schedules/batch 只允许修改时间和地点字段，外加 dayShift 做日期整体平移；classId 不可批量篡改，date 也不能直接赋值（只能用 dayShift 相对平移，否则整批课会被压到同一天）',
      ...
      'rate limiting：POST /api/auth/register 启用 60 次/分钟限速；POST /api/auth/login 启用 10 次/15 分钟限速（防暴力破解）；PUT /api/auth/api-key 与 PUT /api/auth/password 受 auth 限速保护（60 次/分钟）；GET /api/backup 启用 10 次/分钟限速；GET /api/schedule-image（含 /monthly、/yearly）启用 30 次/分钟限速；所有认证后数据修改端点（POST/PUT/DELETE）启用 100 次/分钟限速',
    ...
    validationRules: {
      ...
      classes: {
        create: { name: '必填，最多100字符', grade: '枚举：初一/初二/初三/高一/高二/高三/大学', subject: '必填，最多50字符', studentCount: '≥1整数', unitPrice: '可选≥0', discountAmount: '可选≥0', discountReason: '可选，最多500字符', isCompetition: '可选布尔', defaultLocationName: '可选，最多200字符', defaultLocationLat: '可选数字', defaultLocationLng: '可选数字' },
        update: { name: '可选（不为空，最多100字符）', grade: '可选枚举', subject: '可选（不为空，最多50字符）', studentCount: '可选≥1', unitPrice: '可选≥0', discountAmount: '可选≥0', discountReason: '可选，最多500字符', isCompetition: '可选布尔', defaultLocationName: '可选，最多200字符', defaultLocationLat: '可选数字', defaultLocationLng: '可选数字' },
        addStudent: { name: '必填，最多100字符', birthDate: '有效 YYYY 或 YYYY-MM-DD（可选，空字符串等同不填）', phone: '可选 11 位手机号', parentPhone: '可选 11 位手机号', parentName: '可选，最多100字符', note: '可选，最多2000字符' },
      },
      students: {
        create: { name: '必填，最多100字符', phone: '11位手机号（可选，空字符串等同不填）', parentPhone: '11位手机号（可选，空字符串等同不填）', birthDate: '有效 YYYY 或 YYYY-MM-DD（可选，空字符串等同不填）', classIds: '整数数组（可选，最多500项）', parentName: '可选，最多100字符', note: '可选，最多2000字符' },
        update: { name: '可选，最多100字符', phone: '可选11位', parentPhone: '可选11位', parentName: '可选，最多100字符', birthDate: '可选有效 YYYY 或 YYYY-MM-DD', note: '可选，最多2000字符', classIds: '可选整数数组，最多500项（无效班级 ID 静默跳过）' },
      },
      schedules: {
        create: { classId: '整数必填', date: 'YYYY-MM-DD 必填', startTime: 'HH:MM 必填（00:00-23:59）', endTime: 'HH:MM 必填（推荐 00:00-23:59，可传 24:00-47:59；总时长须大于0且小于24小时）', durationBilling: '可选非负整数（默认由 endTime-startTime 自动计算）', locationName: '可选字符串，最多200字符（默认继承班级默认地点）', locationLat: '可选数字', locationLng: '可选数字' },
        update: { classId: '可选整数（变更时校验归属和软删除）', date: '可选 YYYY-MM-DD', startTime: '可选 HH:MM（00:00-23:59）', endTime: '可选 HH:MM（推荐 00:00-23:59，可传 24:00-47:59；总时长须大于0且小于24小时）', durationBilling: '可选非负整数（startTime/endTime 变更且未显式传时自动重算）', locationName: '可选字符串，最多200字符', locationLat: '可选数字或null', locationLng: '可选数字或null' },
        batchCreate: { classId: '整数必填', startTime: 'HH:MM 必填（00:00-23:59）', endTime: 'HH:MM 必填（推荐 00:00-23:59，可传 24:00-47:59；总时长须大于0且小于24小时）', dates: '可选数组,最多 365 项', semesterId: '可选,与 weekday 配合学期模式', weekday: '可选 0-6,与 semesterId 配合', durationBilling: '可选非负整数（默认由 endTime-startTime 自动计算）', preview: '布尔值可选（默认 false,true 时仅返回 {count,dates} 预览不创建）', crossSemester: '布尔值可选；确认允许指定日期跨学期边界' },
        ...
        batchDelete: { ids: '整数数组（byIds模式,最多 500 项）', start: 'YYYY-MM-DD（byDateRange模式必填）', end: 'YYYY-MM-DD（byDateRange模式必填）', classId: '正整数（byClassId模式必填;byDateRange模式可选过滤）', fromDate: 'YYYY-MM-DD（byClassId模式必填）', semesterOnly: 'boolean 可选（默认 true,仅删学期内排课）', dryRun: 'boolean 可选（默认 false,true 时仅预览不删除）' },
      },
      semesters: {
        create: { name: '必填', type: '枚举：spring/fall/summer/winter（对应 春季/秋季/暑假/寒假）', startDate: 'YYYY-MM-DD 必填', endDate: 'YYYY-MM-DD 必填,须不早于 startDate' },
      ...
      holidays: {
        ...
        batch: { items: '数组,最多 365 项;每项须有有效 date 和 holiday|workday type;同一 date 仅插入第一条（重复跳过,返回 {count, skipped}）' },
      },
      pricingTiers: {
    ...
    examples: {
      ...
      '把周四的课统一挪到周五（先 dryRun 预览）': 'PUT /api/schedules/batch {"classId":1,"weekday":4,"updates":{"dayShift":1}}',
