...
router.get('/agent/help', (req, res) => {
  res.json({
    ...
    notes: [
      ...
      '审计日志（audit_log）覆盖：排课 CREATE/UPDATE/DELETE/BATCH_DELETE/BATCH_UPDATE；班级 CREATE/UPDATE/DELETE（含恢复操作）；学生 CREATE/UPDATE/DELETE（含子路由 POST/DELETE /api/classes/:cid/students）；节假日 CREATE/UPDATE/DELETE/BATCH_CREATE；定价阶梯 CREATE/UPDATE/DELETE；学期 CREATE/UPDATE/DELETE；教师（teachers）API Key 轮换/密码修改/学科更新。日志超过 10000 条时自动删除最旧记录',
