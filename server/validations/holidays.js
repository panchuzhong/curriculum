...
export const validateCreateHoliday = [
  ...
  body('name').notEmpty().isLength({ max: 100 }).withMessage('名称不能为空'),
];
...
export const validateUpdateHoliday = [
  ...
  body('name').optional().notEmpty().isLength({ max: 100 }).withMessage('名称不能为空'),
];
...
export const validateBatchHolidays = [
  ...
  body('items.*.name').optional().isLength({ max: 100 }),
];
