...
// Settings cannot drift from the scheduling and image-export paths.
export const BUILT_IN_HOLIDAYS = Object.fromEntries(
  Object.keys(HOLIDAY_DATES).map(year => [year, [
    ...HOLIDAY_DATES[year].map(mmdd => ({
      date: `${year}-${mmdd}`, type: 'holiday', name: HOLIDAY_NAMES[mmdd] || '节假日',
    })),
    ...(WORKDAY_DATES[year] || []).map(mmdd => ({
      date: `${year}-${mmdd}`, type: 'workday', name: '调休上班',
    })),
  ]]),
);
...
export default function HolidayManager() {
  ...
  function reload() {
    api.getHolidays().then(setHolidays).catch(e => toast(e.message || '加载节假日失败'));
  }
